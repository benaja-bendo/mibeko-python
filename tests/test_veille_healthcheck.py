"""Ping healthcheck de la veille (mibeko-python#21) : no-op par défaut, ne
lève jamais — un ping raté ne doit pas faire échouer la veille elle-même.
"""

import httpx

from src.veille import healthcheck


def test_ping_noop_si_url_non_configuree(monkeypatch):
    monkeypatch.delenv("VEILLE_HEALTHCHECK_URL", raising=False)
    called = []
...
def test_ping_appelle_lurl_configuree_avec_le_suffixe(monkeypatch):
    monkeypatch.setenv("VEILLE_HEALTHCHECK_URL", "https://hc.example/ping/abc123")
    appels = []
