...
def list_documents(
    ...
    q = db.query(LegalDocument).order_by(LegalDocument.created_at.desc())
    if status:
        q = q.filter(LegalDocument.extraction_status == status)
    documents = q.offset(offset).limit(limit).all()
    return [_build_summary(doc) for doc in documents]
...
def global_stats(db: Session = Depends(get_db)):
    ...
    completed = db.query(func.count(LegalDocument.id)).filter(LegalDocument.extraction_status == "completed").scalar() or 0
    processing = db.query(func.count(LegalDocument.id)).filter(LegalDocument.extraction_status == "processing").scalar() or 0
    pending = db.query(func.count(LegalDocument.id)).filter(LegalDocument.extraction_status == "pending").scalar() or 0
    failed = db.query(func.count(LegalDocument.id)).filter(LegalDocument.extraction_status == "failed").scalar() or 0
