...
- **`DELETE /api/v1/documents/{id}` est réversible** : il soft-delete le document et ses articles, sans toucher aux versions, nœuds, runs, `media_files` ni objets MinIO. `POST /api/v1/documents/{id}/restore` restaure le document et les articles retirés avec lui ; les articles supprimés antérieurement restent supprimés. La purge physique n'est pas exposée par le service Python.
...
- **L'OCR par défaut est Mistral OCR, pas MinerU, depuis le 14/09/2026** (`mibeko-python#22`, `docs/decisions.md`) : `OCR_BACKEND=mistral` (défaut de `src/parsing/batch.py`), `src/services/mistral_ocr_service.py`, modèle épinglé `MISTRAL_OCR_MODEL` (jamais `-latest`). `OCR_BACKEND=mineru` reste un repli explicite (dev, incident) vers `mineru_service`/`MINERU_BACKEND`, inchangé. Le triage (`src/parsing/triage.py`) route désormais aussi vers l'OCR quand plusieurs pages ont un ratio de mots d'une lettre élevé (couche OCR pré-existante dégradée, cf. `MAX_SINGLE_LETTER_WORD_RATIO`/`MIN_DEGRADED_PAGES`) — pas seulement sur l'absence de texte natif. Aucune normalisation d'en-tête supplémentaire n'était nécessaire pour lire les sorties Mistral OCR (ATX `#`/`##` ou gras) : `_MD_PREFIX`/`_MD_SUFFIX` (`src/extractor/parser.py`) les strippaient déjà avant `ARTICLE_PATTERN`, vérifié en rejouant `LegalDocumentParser` sur une vraie sortie Mistral OCR (766 articles reconnus sur 68 pages, un seul chiffre d'écart).
